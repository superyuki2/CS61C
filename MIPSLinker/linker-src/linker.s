...
write_machine_code:
	...
	sw $s3, 16($sp)
	sw $s4, 12($sp) 
	sw $s5, 8($sp) #CHANGE THE EPILOGUE! I USE THIS TO HOLD BYTE OFFSET
	sw $s6, 4($sp) #we have up to $s7 reminder; I use this to hold the numerical form of the instruction
	sw $ra, 0($sp)
...
write_machine_code_find_text:	
	move $a0, $s1 #takes the input file pointer
	jal readline
	blt $v0, $0, write_machine_code_error	# Invalid line reached
	move $a0, $v1 #the line value moved to $a0 NOTE THAT V1 IS THE ONE USED FOR THE STRING IN THE NEW LINE
	la $a1, textLabel #compare the line value with textLabel
	jal streq
	bne $v0, $0, write_machine_code_find_text #recurse the whole thing again if the text section not found yet
...
write_machine_code_next_inst:
	...
	li $a1, 16
	jal parse_int
	move $s6, $v0 #moves the converted instruction to a temporary register
	
	# 4. Check if the instruction needs relocation. If it does not, branch to
	# the label write_machine_code_to_file:
	move $a0, $s6 #moves the manipulable numerical form of the instruction into the argument for calling 
	jal inst_needs_relocation
	beq $v0, $0, write_machine_code_to_file #NO RELOCATION NEEDED! CAN DIRECTLY WRITE TO OUTPUT
	...
	#$a0 is already a manipulable number as instruction
	move $a1, $s5 #byte offset
	move $a2, $s2
	move $a3, $s3
	jal relocate_inst
	move $s6, $v0 #$s6 is supposed to hold the numeric instruction: this works 
	li $t0, -1
	beq $v0, $t0, write_machine_code_error
	
write_machine_code_to_file:
	# 6. Write the instruction into a string buffer via hex_to_str():
	move $a0, $s6 #$s6 has changed so we need to reload into the argument
	la $a1, hex_buffer # Set $a1 = the buffer that we will write.
	jal hex_to_str #writes into the space held by hex_buffer
	
	# 7. Increment the byte offset by the appropriate amount:
	add $s5, $s5, 4 #instructions are word aligned
