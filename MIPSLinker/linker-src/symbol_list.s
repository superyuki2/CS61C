...
.text
	#linked list initialized in test
...
#  $a1 = name to look for (string, char pointer) because registers only hold 32 bits (4 bytes) we can assume it is 
#
# Returns:  address of symbol if found or -1 if not found
#------------------------------------------------------------------------------
addr_for_symbol:
	beq $a0, $0, addr_not_found	# Begin addr_for_symbol
	#prologue
	addi $sp, $sp, -12 #save registers for later
	sw $s0, 8($sp)
	sw $s1, 4($sp)
	sw $ra, 0($sp)
	move $s0, $a0 #save the argument of the list pointer
	move $s1, $a1 #save the argument of the string (char pointer)
	lw $t0, 4($a0) #t0 = head.name
	move $a0 $t0 #set arguments to compare the string we are looking for to 
	move $a1 $a1

	jal streq #will have return value register of 0 if they are equal

	move $a0, $s0 #restore the argument parameters
	move $a1, $s1
	lw $s0, 8($sp)	#epilogue to restore previous saved parameteres (in case we exit to one of the two conditions and $s0 not restored)
	lw $s1, 4($sp)
	lw $ra, 0($sp)
	addiu $sp, $sp, 12 #move stack back up before continuing to recurse

	beq $v0, $0, addr_found #$v0 value comes from streq
	lw $a0, 8($a0)	#move on to the next pointer
	j addr_for_symbol
addr_found: 
	lw $v0, 0($a0)#return the address
	jr $ra
addr_not_found:
	li $v0, 0
...
add_to_list:	
	...
	move $s2, $v0	# s2 = copy of string held at a2 (symbol name)
	
	jal new_node	# allocate space for new ListNode (12 bytes blank heap space)
	sw $s1, 0($v0)	# set node.addr = addr
	sw $s2, 4($v0)	# set node.name = copy of name
	sw $s0, 8($v0)	# set node.next = head
add_to_list_exit:	#epilogue
	lw $s0, 12($sp)	
	lw $s1, 8($sp)
	lw $s2, 4($sp)
	lw $ra, 0($sp)
