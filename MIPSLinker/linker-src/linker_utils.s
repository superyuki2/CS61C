...
inst_needs_relocation:
	# YOUR CODE HERE
	srl $t0, $a0, 26 #shift right to obtain opcode
	addi $t1, $0, 2 #check for j 
	beq $t1, $t0, relocate
	addi $t2, $0, 3 #check for jal
	beq $t2, $t0, relocate
	j not_relocate
relocate:
	addi $v0, $0, 1	
...
relocate_inst:
        ...
        sw $a3, 12($sp)
        sw $ra, 16($sp)
        lw $a0, 12($sp) #load the relocation table into a0
        jal symbol_for_addr
        beq $v0, $0, no_addr_or_symbol #return if symbol name is null
        add $a1, $v0, $0 #symbol name as a1
        lw $a0, 8($sp) #symbol table as a0
        jal addr_for_symbol
	li $t0, -1 
        beq $v0, $t0, no_addr_or_symbol #again, null check
relocation:
	lw $t0, 0($sp) #load instruction into t0
	andi $v0, $v0, 0x0fffffff #mask out the PC + 4  first 4 bits
	srl $v0, $v0, 2 #shift right symbol addr 2 bits
	andi $t0, $t0, 0xfc000000 #mask out first 26 bits
	or $v0, $t0, $v0 #or mask 6 MSB of instruction and v0
        j epilogue
no_addr_or_symbol:
        addi $v0, $0, -1 #return -1 if error
epilogue:
        lw $a0, 0($sp) #epilogue 
        lw $a1, 4($sp)
        lw $a2, 8($sp)
