...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    ...
    /* Since we pass this buffer to strtok(), the characters in this buffer will
       GET CLOBBERED. */
    char buf[BUF_SIZE]; //2^10 is buf size
    /* Store input line number / byte offset. When should each be incremented? when you go to the next instruction*/
    uint32_t input_line = 0; //start with zero offset
    uint32_t byte_offset = 0;
    int ret_code = 0;

    /* First, read the next line into a buffer. */
    while (fgets(buf, BUF_SIZE, input)) {
        input_line++;//increments the line; note that the newline in instructions makes fgets go to the next line
        byte_offset += 4;
        ...
        char* args[MAX_ARGS];
        int num_args = 0;
        int test = parse_args(input_line, args, &num_args);
        if (test == -1) {
            ret_code = -1;
        }
        ...
        /* Use translate_inst() to translate the instruction and write to output file.
           ...
           raise_inst_error(). */
        int transerror = translate_inst(output, name, args, num_args, byte_offset, symtbl, reltbl);
        if (transerror == -1) {
        	raise_inst_error(input_line, token, args, num_args);
        	retcode = -1;
        }
